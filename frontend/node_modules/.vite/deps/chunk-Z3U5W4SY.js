...
var replaceNames = (builder, cache) => {
  ...
  for (const command of builder.commands) {
    if ((_a = command.MakeMoveVec) == null ? void 0 : _a.type) {
      if (!command.MakeMoveVec.type.includes(NAME_SEPARATOR))
        continue;
    ...
    for (let i = 0; i < types.length; i++) {
      if (!types[i].includes(NAME_SEPARATOR))
        continue;
...
var namedPackagesPlugin = ({
  ...
  async function query(client, requests) {
    ...
    for (const req of requests) {
      ...
      if (req.type === "package")
        results.packages[req.name] = data.address;
      if (req.type === "moveType")
        results.types[req.name] = data.repr;
    }
    return results;
  }
};
...
//# sourceMappingURL=chunk-Z3U5W4SY.js.map
